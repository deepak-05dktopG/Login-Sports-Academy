  ...
  try {
    ...
    if (oldEntry.amount && oldEntry.paymentType) {
      ...
      if (revertAmount !== 0 || revertExpense !== 0 || revertWithdrawal !== 0) {
        await incrementCashBox({ amount: revertAmount, paymentType: oldEntry.paymentType, expenseDelta: revertExpense, withdrawalDelta: revertWithdrawal, entryType: oldEntry.type, entryCountDelta: -1, entryTotalDelta: -oldEntry.amount, serviceType: oldEntry.serviceType || 'swimming' });
      }
    }
    ...
    if (updatedEntry.amount && updatedEntry.paymentType) {
      ...
      if (applyAmount !== 0 || applyExpense !== 0 || applyWithdrawal !== 0) {
        await incrementCashBox({ amount: applyAmount, paymentType: updatedEntry.paymentType, expenseDelta: applyExpense, withdrawalDelta: applyWithdrawal, entryType: updatedEntry.type, entryCountDelta: 1, entryTotalDelta: updatedEntry.amount, serviceType: updatedEntry.serviceType || 'swimming' });
      }
    }
...
export const deleteDailyTrackerById = async (req, res) => {
  try {
    ...
    if (entry.amount && entry.paymentType) {
      ...
      if (revertAmount !== 0 || revertExpense !== 0 || revertWithdrawal !== 0) {
        await incrementCashBox({ amount: revertAmount, paymentType: entry.paymentType, expenseDelta: revertExpense, withdrawalDelta: revertWithdrawal, entryType: entry.type, entryCountDelta: -1, entryTotalDelta: -entry.amount, serviceType: entry.serviceType || 'swimming' });
      }
    }
