...
const createMembersForDraft = async (
	...
	for (const m of membersToCreate) {
		...
		const qrPayload = `lsa:member:${doc._id.toString()}`
...
const finalizePaymentAndCreateMembers = async ({ paymentDoc, providerPaymentId, providerSignature }) => {
	...
	// Stamp payment fields on all created members for online payments
	...
	if (createdMembers.length > 0) {
		await Member.updateMany(
			...
			{ $set: { paidAmount: paidNow, pendingAmount: 0, paymentStatus: 'paid' } }
		);
...
// Validates and normalizes a public batch time slot (date, start/end times) for facility entry
...
const normalizePlanForClient = planDoc => {
	...
	const result = {
		...
		type,
		serviceType: raw?.serviceType || 'swimming',
...
// Seeds the official Login Sports Academy plans (Swimming + Badminton) into the database (upserts to avoid duplicates)
export const seedOfficialPlans = asyncHandler(async (req, res) => {
	...
	const officialPlans = [
		...
		// ===== BADMINTON PLANS =====
		{
			planName: 'Badminton Coaching (Monthly)',
			type: 'monthly',
		...
		{
			planName: 'Badminton Monthly Membership',
			type: 'monthly',
			serviceType: 'badminton',
			categoryRequired: false,
			durationInDays: 30,
		...
		{
			planName: 'Badminton 6 Months Membership',
			type: 'monthly',
			serviceType: 'badminton',
			categoryRequired: false,
			durationInDays: 180,
		...
		{
			planName: 'Badminton Yearly Membership',
			type: 'yearly',
			serviceType: 'badminton',
			categoryRequired: false,
			durationInDays: 365,
	...
	for (const plan of officialPlans) {
		const resUpsert = await MembershipPlan.updateOne(
			{ type: plan.type, serviceType: plan.serviceType || 'swimming', planName: plan.planName },
			{ $set: plan },
			{ upsert: true }
		)
		upserted.push({ planName: plan.planName, type: plan.type, serviceType: plan.serviceType, inserted: Boolean(resUpsert.upsertedId) })
	}
...
export const verifyRazorpayPayment = asyncHandler(async (req, res) => {
	...
	try {
		...
		await DailyTracker.create({
			...
			serviceType: out?.plan?.serviceType || 'swimming',
		...
		if (providerOrderId && providerPaymentId) {
			...
			if (paymentDoc && paymentDoc.status !== 'paid') {
				...
				if (expectedPaise && Number(entity?.amount) === expectedPaise) {
					try {
						const out = await finalizePaymentAndCreateMembers({ paymentDoc, providerPaymentId })
						
						// Add to DailyTracker (webhook fallback)
						try {
							const indiaNow = getIndiaNow();
							const finalAmount = (paymentDoc.pricing && typeof paymentDoc.pricing.total === 'number')
								? paymentDoc.pricing.total
							...
							await DailyTracker.create({
								...
								date: indiaNow.date,
								time: indiaNow.time,
								notes: `Plan: ${out?.plan?.planName || out?.plan?.name || ''} (Webhook)`,
								serviceType: out?.plan?.serviceType || 'swimming',
								memberId: out?.member?._id,
								paymentId: paymentDoc._id,
							});
...
export const listMembers = asyncHandler(async (req, res) => {
	const { q, status, paymentStatus, planType, planId, serviceType, page, limit, sort, order } = req.query
	const filter = {}
	...
	const [total, members] = await Promise.all([
		...
		Member.find(filter)
			.populate('planId', 'planName type basePrice serviceType')
			.sort(sortObj)
			.skip(skip)
			.limit(limitNum),
	])
