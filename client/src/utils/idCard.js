...
const brandPrefix = () => {
    const base = (BRAND.shortName || BRAND.name || 'academy')
    return String(base)
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/(^-|-$)/g, '')
...
const brandIdPrefix = () => {
    const source = String(BRAND.shortName || BRAND.name || '').trim()
    ...
    return (safe || 'ID').slice(0, 3)
}

// Generates the member ID card as a PNG data URL with name, QR code, plan, and dates
export const buildMemberIdCardPng = async (
    ...
    const brandLine = String(BRAND.shortName || BRAND.name || 'ACADEMY').toUpperCase()
    const brandX = logo ? 116 : 60
    const brandMaxW = canvas.width - brandX - 60
    ctx.fillStyle = '#00FFD4'
    ctx.font = '900 24px system-ui, -apple-system, Segoe UI, Roboto, Arial'
    ctx.fillText(ellipsizeToWidth(ctx, brandLine, brandMaxW), brandX, 72)
    ctx.fillStyle = 'rgba(255,255,255,0.70)'
    ctx.font = '700 13px system-ui, -apple-system, Segoe UI, Roboto, Arial'
    const tagline = String(BRAND.tagline || 'Member Card')
    ctx.fillText(ellipsizeToWidth(ctx, tagline, brandMaxW), brandX, 94)
    ...
    // Fields
    const customerId = `${brandIdPrefix()}-${safeIdSuffix(memberId, 8).toUpperCase()}`
...
export const downloadMemberIdCard = async ({ name, memberId, qrDataUrl, planName, joinDate, expiryDate }) => {
    ...
    const filename = `${brandPrefix()}-id-${safeIdSuffix(memberId, 8)}.png`
